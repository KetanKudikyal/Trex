...
export class OracleManagerPrivate {
   ...
   */
  async verifyPaymentProof(proof: PaymentProof): Promise<any> {
    try {
   ...
   */
  async completePrivateVerificationFlow(
    ...
    try {
      ...
      console.log('🔐 OracleManagerPrivate - Parameters being sent to backend:')
   ...
   */
  async getMessageDetails(msgHash: string): Promise<any> {
    try {
      const response = await fetch(`${this.baseUrl}/message/${msgHash}/details`)
      const data = await response.json()
   ...
   */
  async getCBTCBalance(userAddress: string): Promise<string> {
    try {
      const response = await fetch(
   ...
   */
  async getRewardBalance(userAddress: string): Promise<string> {
    try {
      const response = await fetch(
   ...
   */
  async getTotalLiquidityProvided(userAddress: string): Promise<string> {
    try {
      const response = await fetch(
   ...
   */
  async loadTokenInfo(userAddress: string | null = null): Promise<any> {
    try {
